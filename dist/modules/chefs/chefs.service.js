...
let ChefsService = class ChefsService {
    ...
    async createProfile(userId, dto) {
        ...
        if (existing)
            throw new common_1.ConflictException('Chef profile already exists');
    ...
    async updateProfile(userId, dto) {
        ...
        if (!chef)
            throw new common_1.NotFoundException('Chef profile not found');
        const { cuisineTypes, mealCategories, signatureDishes, serviceTypes, pricing, availability, ...scalar } = dto;
        const minRate = pricing && pricing.length ? Math.min(...pricing.map((p) => p.rate)) : undefined;
    ...
    async search(dto) {
        const { q, cuisine, mealCategory, serviceType, hireType, location, minRating, sortBy, page = 1, limit = 20, } = dto;
        const where = { status: 'approved' };
        if (cuisine)
            where.cuisineTypes = { some: { cuisine } };
        if (mealCategory)
            where.mealCategories = { some: { mealCategory } };
        if (serviceType)
            where.serviceTypes = { some: { serviceType } };
        if (hireType)
            where.pricing = { some: { hireType } };
        if (location)
    ...
    async addBlockOutDate(userId, date) {
        const chef = await this.prisma.chef.findUnique({ where: { userId } });
        if (!chef)
            throw new common_1.NotFoundException('Chef profile not found');
    ...
    async removeBlockOutDate(userId, date) {
        const chef = await this.prisma.chef.findUnique({ where: { userId } });
        if (!chef)
            throw new common_1.NotFoundException('Chef profile not found');
    ...
    async pinPost(userId, postId) {
        ...
        if (chef.pinnedPosts.length >= 3)
            throw new common_1.ForbiddenException('Maximum 3 pinned posts allowed');
    ...
    async updateRating(chefId, newRating, reviewCount) {
